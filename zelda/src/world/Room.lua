...
function Room:generateObjects()
    ...
    local numPots = math.random(POTS_PER_ROOM_MIN, POTS_PER_ROOM_MAX)
    for x = 1, numPots do
        ...
        -- find a new random spot for the pot if it collides with the player so we don't start inside a pot
        while self.player:collides(pot) do
            pot.x, pot.y =
            math.random(MAP_RENDER_OFFSET_X + TILE_SIZE,
                        VIRTUAL_WIDTH - TILE_SIZE * 2 - 16),
            math.random(MAP_RENDER_OFFSET_Y + TILE_SIZE,
        ...
        if entity.health <= 0 and not entity.dead then
            ...
            if math.random() <= HEART_DROP_CHANCE then
                ...
                heart.onCollide = function()
                    self.player.health = math.min(
                        self.player.health + PLAYER_HEAL_AMOUNT,
                        PLAYER_MAX_HEALTH
                    )
                end
