...
function Paddle:init(skin)
    ...
    -- Paddle size score: If it goes until a certain threshold, grow the paddle
    self.secretScore = 0
end
...
--[[
    Handles paddle size changing including width
]]
...
--[[
    Applies paddle's secret scoring for size growing purposes
]]
function Paddle:addToScore(scoreIncrease)
    self.secretScore = self.secretScore + scoreIncrease

    if self.size < 4 and self.secretScore > PADDLE_SECRET_SCORE_THRESHOLD then
        self:changeSize(self.size + 1)
    end
end

function Paddle:dropSize()
    local newSize = math.max(1, self.size - 1)
