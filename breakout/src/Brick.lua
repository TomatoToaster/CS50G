...
function Brick:init(x, y)
    ...
    -- used to determine whether this brick is a locked brick
    self.isLocked = false
...
function Brick:hit()
    ...
    -- Only emit particles and change/destory the brick if it is not locked
...
function Brick:render()
    if self.inPlay then
        local brickIdx = self.isLocked and 22 or (1 + ((self.color - 1) * 4) + self.tier)

        love.graphics.draw(gTextures['main'],
            -- multiply color by 4 (-1) to get our color offset, then add tier to that
            -- to draw the correct tier and color brick onto the screen
            gFrames['bricks'][brickIdx],
            self.x, self.y)
    end
end
