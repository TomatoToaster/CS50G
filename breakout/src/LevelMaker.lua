...
function LevelMaker.createMap(level, hasLockKey)
    local bricks = {}
    ...
    if #bricks == 0 then
        return self.createMap(level, hasLockKey)
    end

    -- if we have a lockKey situation, then randomly lock a brick
    if hasLockKey then
        local lockedIdx = math.random(1, #bricks)
        bricks[lockedIdx].isLocked = true
    end

    return bricks
end

function LevelMaker.createPowerups(level, hasLockKey)
    local powerups = {}
    local amount = math.floor(level / 6)

    for i = 1, amount or 2 do
        -- For now we're just using the bonus ball powerup so hard code it
        local type = 9

        powerups[i] = Powerup(type)
    end

    if hasLockKey or false then
        -- Add in the key powerup
        table.insert(powerups, Powerup(10))
    end
