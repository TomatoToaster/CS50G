...
function PaddleSelectState:update(dt)
    ...
    if love.keyboard.wasPressed('return') or love.keyboard.wasPressed('enter') then
        ...
        local withLockKey = math.random(LOCK_KEY_CHANCE) == 1
        gStateMachine:change('serve', {
            paddle = Paddle(self.currentPaddle),
            bricks = LevelMaker.createMap(START_LEVEL, withLockKey),
            powerups = LevelMaker.createPowerups(START_LEVEL, withLockKey),
            health = 3,
            score = 0,
