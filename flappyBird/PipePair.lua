...
-- Pipe Min/ Maxs
local GAP_LENGTH_MIN = 120
local GAP_LENGTH_MAX = 90
...
function PipePair:init(y)
    ...
    -- get a random gap length
    local gapLength = math.random(GAP_LENGTH_MIN, GAP_LENGTH_MAX)

    -- instantiate two pipes that belong to this pair
    self.pipes = {
        ['upper'] = Pipe('top', self.y),
        ['lower'] = Pipe('bottom', self.y + PIPE_HEIGHT + gapLength)
    }
