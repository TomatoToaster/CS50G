...
function PlayerJumpState:update(dt)
    ...
    for k, object in pairs(self.player.level.objects) do
        if object:collides(self.player, true, true) then
            -- if the object is solid, to collide it should also be above the player's head with some leeway (-3)
            if object.solid and object.y + object.height - 3 <= self.player.y then
    ...
    for k, entity in pairs(self.player.level.entities) do
        if entity:collides(self.player, true) and not self.player.hasWon then
            gSounds['death']:play()
