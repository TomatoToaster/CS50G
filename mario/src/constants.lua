...
-- player collision leeway: how many pixels from the edge we check so we can fall between tiles, originally 3
...
-- how much the level grows per win
LEVEL_GROWTH = 5

-- How long to dance in seconds after winning
DANCE_TIME = 3
...
for i = 1, 30 do
    -- Skip every other block so they next one can be the crumbled/consumable version
    if i % 2 == 1 then
